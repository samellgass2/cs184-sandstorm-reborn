...
void sandSimulator::load_textures() {
...
void sandSimulator::load_shaders() {
...
sandSimulator::~sandSimulator() {
  ...
  if (sandbox) delete sandbox;
  if (sp) delete sp;
  if (collision_objects) delete collision_objects;
}

void sandSimulator::loadSandbox(Sandbox *sandbox) { this->sandbox = sandbox; }

void sandSimulator::loadSandparameters(SandParameters *sp) { this->sp = sp; }
...
void sandSimulator::init() {
  ...
  for (auto &pm : sandbox->sand_particles) {
    avg_pm_position += pm.position / sandbox->sand_particles.size();
  }
...
//          2 * sandbox->num_width_points * sandbox->num_height_points -
//          sandbox->num_width_points - sandbox->num_height_points;
//  int num_shear_springs =
//          2 * (sandbox->num_width_points - 1) * (sandbox->num_height_points - 1);
//  int num_bending_springs = num_structural_springs - sandbox->num_width_points -
//                            sandbox->num_height_points;
//
...
//void sandSimulator::drawNormals(GLShader &shader) {
...
void sandSimulator::drawPhong(GLShader &shader) {
  ...
  //TODO: by vertex approach : keeping in case needed
//  int num_vertices = 0;
//  for (auto sand : sandbox->sand_particles) {
//    num_vertices += sand.m_sphere_mesh.sphere_num_vertices;
//  }
//
//  MatrixXf positions(4, num_vertices);
//  MatrixXf normals(4, num_vertices);
//  MatrixXf uvs(2, num_vertices);
//  MatrixXf tangents(4, num_vertices);

//  for (int i = 0; i < sandbox->sand_particles.size(); i++) {
//    Triangle *tri = cloth->clothMesh->triangles[i];
//
//    Vector3D p1 = tri->pm1->position;
//    Vector3D p2 = tri->pm2->position;
...
bool sandSimulator::cursorPosCallbackEvent(double x, double y) {
...
void sandSimulator::mouseMoved(double x, double y) { y = screen_h - y; }

void sandSimulator::mouseLeftDragged(double x, double y) {
  float dx = x - mouse_x;
  float dy = y - mouse_y;
...
bool sandSimulator::dropCallbackEvent(int count, const char **filenames) {
  return true;
}
...
bool sandSimulator::resizeCallbackEvent(int width, int height) {
  screen_w = width;
  screen_h = height;
